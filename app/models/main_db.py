...
class MainDB(Base):
    ...
    DB_EST_CAT = Column(
        String(100), nullable=True, index=True, comment="Establishment Category"
    )
    ...
    DB_PROD_CLASS_PRESCRIP = Column(
        Text, nullable=True, comment="Prescription Classification"
    )
    ...
    DB_PROD_DISTRI_LTO_NO = Column(
        Text, nullable=True, comment="Distributor LTO Number"
    )
    ...
    DB_TYPE_DOC_RELEASED = Column(
        Text, nullable=True, comment="Type of Document Released"
    )
    ...
    DB_CPR_COND_ADD_REMARKS = Column(
        Text, nullable=True, comment="CPR Condition Additional Remarks"
    )

    # Status & Tracking (6 columns)
    DB_APP_STATUS = Column(
        String(50), nullable=True, index=True, comment="Application Status"
    )
    ...
    DB_TRASH_DATE_ENCODED = Column(
        DateTime, nullable=True, comment="Trash Date Encoded"
    )
    DB_USER_UPLOADER = Column(String(255), nullable=True, comment="User Uploader")
    DB_DATE_EXCEL_UPLOAD = Column(
        DateTime,
        nullable=True,
        index=True,
        default=func.now(),
        comment="Date Excel Uploaded",
    )

    # Pharmaceutical Product Category (3 columns)
    DB_PHARMA_PROD_CAT = Column(
        String(255), nullable=True, comment="Pharmaceutical Product Category"
    )
    DB_PHARMA_PROD_CAT_LABEL = Column(
        String(255), nullable=True, comment="Pharmaceutical Product Category Label"
    )
    DB_IS_IN_PM = Column(TINYINT, nullable=True, default=0, comment="Is in PM")

    DB_TIMELINE_CITIZEN_CHARTER = Column(
        Integer, nullable=True, comment="Timeline for Citizen Charter"
    )
    DB_PROCESSING_TYPE = Column(Text, nullable=True, comment="Processing Type")

    DB_DECISION_AUTHORITY = Column(
        String(255), nullable=True, comment="Decision Authority Name"
    )
    DB_DECISION_RESULT = Column(String(255), nullable=True, comment="Decision Result")
    DB_DECISION_SIGNED_DATE = Column(
        Date, nullable=True, comment="Decision Authority Signed Date"
    )
    DB_ENTRY_TYPE = Column(
        String(50),
        nullable=True,
        default="ORIGINAL",
        comment="Entry Type: ORIGINAL, CORRECTION, RECONSTRUCTION",
    )
    DB_SUBJECT = Column(Text, nullable=True, comment="Subject")
    DB_LTR_DATE_RECEIVED_FDAC = Column(
        Date, nullable=True, comment="Letter Date Received From FDAC"
    )
    ...
    def __repr__(self):
        return (
            f"<MainDB(id={self.DB_ID}, dtn={self.DB_DTN}, est_cat={self.DB_EST_CAT})>"
        )
    ...
    application_logs = relationship(
        "ApplicationLogs", back_populates="main_db", cascade="all, delete-orphan"
    )
